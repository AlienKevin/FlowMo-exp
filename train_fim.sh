...
# # Execute torchrun with the calculated port
# torchrun --master_port=$MASTER_PORT -m flowmo.train \
#     --experiment-name "flowmo_qwen2.5-coder-0.5b_span_0.15_pretrain" \
#     model.context_dim=896 model.quantization_type=qwen2.5-coder-0.5b_span_0.15 model.code_length=128 \
...
Execute torchrun with the calculated port
torchrun --master_port=$MASTER_PORT -m flowmo.train \
    --experiment-name "flowmo_qwen2.5-coder-0.5b_span_${span}_pretrain" \
