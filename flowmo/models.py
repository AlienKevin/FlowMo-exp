...
class FlowMo(nn.Module):
    ...
    def _quantize(self, code, cond, caption):
        ...
        if self.config.model.quantization_type == "noop":
            ...
            quantized, (prior_loss, commit_loss, double_quant_loss, per_sample_entropy, codebook_entropy, entropy_aux_loss), indices = self.quantizer(code, cond, caption, prior_stop_grad=self.config.prior.stop_grad)
            assert quantized.shape == code.shape
            quantized = einops.rearrange(quantized, "b fg (t fh) -> b t (fg fh)", t=t)
